...
def food_heuristic(state, problem):
    ...
    foodList = foodGrid.as_list()
    gameState = problem.starting_game_state
    ...
    for food in foodList:
        ...
        if distance < threshold:
            threshold = distance
            minDistance = maze_distance(food, position, gameState) #only compute when new closer food (avoid calls to maze distance)
